...
const ConversationPromptPage: React.FC = () => {
  ...
  const { mutate: createConversation, isPending } = useMutation({
    ...
    onError: (err: any) => {
      notifyError(err?.response?.data?.message || "Thất bại khi tạo lời nhắc");
    },
  });
  ...
  const { mutate: editConversation, isPending: updating } = useMutation({
    ...
    onError: (err: any) =>
      notifyError(
        err?.response?.data?.message || "Thất bại khi cập nhật lời nhắc"
      ),
  });
  ...
  return (
    <div>
      ...
      >
        <Form layout="vertical" form={form} onFinish={handleCreate}>
          <Form.Item
            label="Tên"
            name="promptName"
            rules={[{ required: true, message: "Vui lòng nhập tên lời nhắc" }]}
          >
      ...
      <Drawer
        width={640}
        title={`Chỉnh sửa lời nhắc: ${selectedPrompt?.promptName}`}
        open={editVisible}
