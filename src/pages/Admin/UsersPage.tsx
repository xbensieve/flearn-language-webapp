...
import { Table, Card, Typography, Tag, Avatar, Button, Drawer, Descriptions, Spin, Select, Space, Empty } from 'antd';
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (status: boolean) => (
        <Tag color={status ? 'success' : 'error'} className="rounded px-2 text-[10px]">
      ...
      <Card bordered={false} className="rounded-2xl shadow-sm border border-gray-100">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-3">
             ...
             <div>
               <Title level={4} className="!mb-0 !font-bold text-gray-800">Users Management</Title>
      ...
      >
        {loadingDetail ? (
          ...
          <div className="flex flex-col items-center">
             ...
             />
             <Title level={4} className="!mb-1">{userDetail.userName}</Title>
             <Text type="secondary" className="mb-6 text-sm">{userDetail.email}</Text>
             
             <Descriptions bordered column={1} className="w-full" size="small">
               <Descriptions.Item label="User ID"><Text copyable className="text-xs">{userDetail.userID}</Text></Descriptions.Item>
               <Descriptions.Item label="Full Name">{userDetail.userName}</Descriptions.Item>
               <Descriptions.Item label="Roles">
                 <div className="flex gap-1">
                    
                    {sortRoles(userDetail.roles).map((r: string) => (
                      <Tag key={r} color={getRoleColor(r)}>{r}</Tag>
                    ))}
                 </div>
               </Descriptions.Item>
               <Descriptions.Item label="Status">
                 <Tag color={userDetail.status ? 'green' : 'red'}>{userDetail.status ? 'Active' : 'Inactive'}</Tag>
               </Descriptions.Item>
               <Descriptions.Item label="Email Confirmed">{userDetail.isEmailConfirmed ? 'Yes' : 'No'}</Descriptions.Item>
               <Descriptions.Item label="Joined">{new Date(userDetail.createdAt).toLocaleString()}</Descriptions.Item>
               <Descriptions.Item label="Last Access">{userDetail.lastAccessAt ? new Date(userDetail.lastAccessAt).toLocaleString() : 'Never'}</Descriptions.Item>
             </Descriptions>
          </div>
        ) : null}
      </Drawer>
