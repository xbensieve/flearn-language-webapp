...
const ProgramPage: React.FC = () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['programs'] }); setDrawerOpen(false); }
  });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (val: boolean, r: any) => (
        <Switch size="small" checked={val} onChange={(c) => updateMutation.mutate({ id: r.programId, payload: { status: c, name: r.name } })} />
      )
    },
    {
      title: '',
      ...
      render: (_: any, r: any) => (
        <Space size="small">
          <Button type="text" size="small" icon={<EyeOutlined className="text-sky-500"/>} onClick={() => navigate(`/admin/levels/${r.programId}`)} />
          <Button type="text" size="small" icon={<EditOutlined className="text-blue-500"/>} onClick={() => { setEditingProgram(r); setDrawerOpen(true); }} />
          <Popconfirm title="Delete?" onConfirm={() => deleteMutation.mutate(r.programId)}>
  ...
  return (
    <div className="space-y-5">
      <Card bordered={false} className="rounded-2xl shadow-sm border-0" bodyStyle={{ padding: '20px' }}>
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <Title level={5} className="!mb-0 text-gray-800">Programs</Title>
          ...
          <div className="flex gap-2">
            ...
            />
            <Button type="primary" icon={<PlusOutlined />} onClick={() => { setEditingProgram(null); setDrawerOpen(true); }} className="bg-blue-600 hover:bg-blue-700">
              New
            </Button>
          </div>
        </div>
      ...
      >
        <ProgramForm 
          ...
          onSubmit={(v) => editingProgram ? updateMutation.mutate({ id: editingProgram.programId, payload: v }) : createMutation.mutate(v)}
