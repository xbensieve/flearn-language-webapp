...
const StatCard = ({ title, value, icon, color, bgColor, subtext, onClick }: any) => (
  <Card
    bordered={false}
    className={`h-full shadow-sm hover:shadow-md transition-all duration-300 rounded-2xl border border-gray-100 ${
...
const Admin: React.FC = () => {
  ...
  const chartData =
    userStats?.dailyRegistrations?.map((item) => ({
          ...
          <Card
            ...
            <div style={{ height: 300, width: '100%' }}>
              {chartData.length > 0 ? (
                ...
                <Empty
                  description="No registration data available"
