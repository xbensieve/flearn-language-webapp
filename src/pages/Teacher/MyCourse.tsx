      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-10">
          ...
          <Space wrap className="gap-3">
            ...
            >
              {statusOptions.map((s) => (
                <Option key={s.value} value={s.value} className="font-medium">
                  {s.label}
                </Option>
              ))}
            </Select>
        ...
        {courses.length > 0 ? (
          <>
            <Row gutter={[24, 24]}>
              {courses.map((course) => {
                const canEdit = ["Draft", "Rejected"].includes(
                  course.courseStatus
                );
                const statusColor =
                  statusColors[course.courseStatus] || "default";
          ...
          <div className="flex flex-col items-center justify-center min-h-[60vh] bg-white rounded-3xl shadow-lg p-12 text-center">
            <Empty
              ...
              description={false}
            />
            <Title level={4} className="mt-6 text-gray-800 mb-3">
              {status
                ? `No ${statusOptions
