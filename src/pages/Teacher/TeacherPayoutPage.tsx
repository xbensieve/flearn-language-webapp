...
const TeacherPayoutPage: React.FC = () => {
    ...
    const fetchPayoutRequests = async () => {
      try {
        const response = await axios.get(
          'https://f-learn.app/api/payout-requests/mine?page=1&pageSize=10'
        );
  ...
  const baseColumns = [
      ...
      render: (_: any, record: any) => (
        <Space>
          <div>
            ...
            <div style={{ fontSize: 12, color: '#888' }}>{record.teacherEmail}</div>
          </div>
        </Space>
      ),
    },
    ...
    },
    {
      ...
      dataIndex: 'amount',
      key: 'amount',
      render: (amount: number) => <Text>{amount.toLocaleString('vi-VN')} ₫</Text>,
    },
    {
      title: 'Payout Times',
      key: 'payoutTimes',
    ...
    },
    {
      ...
      key: 'accountNumber',
      render: (accountNumber: number) => <Text>{accountNumber.toLocaleString()}</Text>,
    },
    {
      title: 'Status',
      dataIndex: 'status',
  ...
  return (
    <div style={{ margin: '20px' }}>
      <Card
        title={
          <Title level={3} style={{ color: '#333', fontWeight: 'bold' }}>
