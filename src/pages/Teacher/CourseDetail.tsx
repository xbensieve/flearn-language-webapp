...
const CourseDetail: React.FC = () => {
  ...
  const deleteUnitMutation = useMutation({
    ...
    onError: (error: AxiosError<any>) => {
      notifyError(error.response?.data?.message || "Thất bại khi xóa chương");
    },
  });
  ...
  const createUnitMutation = useMutation({
    ...
    onError: (error: AxiosError<any>) => {
      notifyError(error.response?.data?.message || "That bại khi tạo chương");
    },
  });
  ...
  const updateUnitMutation = useMutation({
    ...
    onError: (error: AxiosError<any>) => {
      notifyError(
        error.response?.data?.message || "Thất bại khi cập nhật chương"
      );
    },
  });
  ...
  if (!course)
    return <Empty description="Không tìm thấy khóa học" className="mt-20" />;
