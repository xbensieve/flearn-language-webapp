...
export default function TeacherApplications() {
  ...
  const [sortOption, setSortOption] = useState<'newest' | 'oldest' | 'name-asc' | 'name-desc'>(
    'newest'
  );
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge className={`${color} border font-medium text-xs px-2 py-0.5 flex items-center gap-1`}>
        ...
        <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
          {isLoading ? (
            ...
            <div className="text-center py-16">
              ...
              <h3 className="text-lg font-medium text-slate-900">Không tìm đơn đăng ký nào</h3>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between">
            <div className="text-sm text-slate-600">
              Page <strong>{page}</strong> of <strong>{totalPages}</strong> ({processedData.length}{' '}
              total)
            </div>
