...
const rolesCase = (roles: string[], navigate: any) => {
  const matchedRole = roles.some((ur) => ur.toLowerCase() === 'learner') ? 'learner' : null;
  if (matchedRole) {
    navigate(ROLE_PATHS[matchedRole]);
  } else {
    notifyError('Please use the Dashboard login portal for staff accounts.');
    localStorage.clear();
    navigate('/login');
  }
};
