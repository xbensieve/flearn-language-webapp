...
const Dashboard = () => {
  ...
  const staffRoom = [
    ...
    { name: "Badiru Pomile", time: "12:35 PM", msg: "Please schedule class test" },
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
            ...
            {classProgress.map((c) => (
              <div key={c.name} className="flex justify-between items-center">
                <div>
                  ...
                  <Text className="block text-gray-400 text-sm">{c.percent}% complete</Text>
                </div>
                <Progress
                  percent={c.percent}
                  size="small"
        ...
        <Card className="shadow-md rounded-xl">
          <div className="flex justify-between items-center mb-3">
            <Title level={5} className="!mb-0">Upcoming Activities</Title>
            <Button type="link" className="!p-0 text-blue-500">See all</Button>
          </div>
          {activities.map((a) => (
            <div key={a.date} className="flex items-center justify-between py-2 border-b last:border-0">
        ...
        <Card className="shadow-md rounded-xl">
          <div className="flex justify-between items-center mb-3">
            <Title level={5} className="!mb-0">Staff Room</Title>
            <Button type="link" className="!p-0 text-blue-500">See all</Button>
          </div>
          {staffRoom.map((s) => (
            <div key={s.name} className="flex justify-between py-2 border-b last:border-0">
              <div>
